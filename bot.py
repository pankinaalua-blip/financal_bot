...
# ---------------------------------------------------------
# 3. СИНХРОННЫЕ ФУНКЦИИ GOOGLE SHEETS
# ---------------------------------------------------------
    ...
    try:
        ...
        if not official_name:
            ...
            ws_ref.append_row([
                ...
                "",
                official_name,
                username_str if username_str else "-",
                user_id_str,
            ])
...
def _sync_batch_settle_debts(person_name: str) -> int:
    """Пакетное списание долгов за один HTTP-запрос через update_cells"""
    ws = spreadsheet.worksheet("Операции")
...
# ---------------------------------------------------------
# 4. АСИНХРОННЫЕ ОБЕРТКИ (БЕЗ БЛОКИРОВКИ EVENT LOOP)
# ---------------------------------------------------------
async def get_or_register_employee(user: types.User) -> str:
    ...
    return await asyncio.to_thread(_sync_get_or_register_employee, user_id_str, username_str, full_name)
...
async def show_project_selection(target_msg, amount: float, merchant: str, data: dict, employee: str):
    ...
    # Индексные callback_data защищают от превышения лимита 64 байт в Telegram
    kb_buttons = [
        [InlineKeyboardButton(text=f"📌 {p[:30]}", callback_data=f"proj_{idx}")]
...
async def msg_my_receipts(message: types.Message):
    ...
    text = (
        f"👤 <b>Чеки к возмещению ({html.escape(name)})</b>\n\n"
...
async def cmd_my_requisites(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[
            InlineKeyboardButton(text="✏️ Изменить / Ввести реквизиты", callback_data="edit_reqs")
        ]]
    )
    await message.answer(
        f"💳 <b>Реквизиты для выплат:</b>\n\n"
        f"👤 <b>Сотрудник:</b> {html.escape(employee)}\n"
...
async def cmd_quick_meal(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=p[:30], callback_data=f"meal_{idx}")]
            for idx, p in enumerate(projects)
        ]
    )
    await message.answer(
...
async def cb_confirm_meal(callback: types.CallbackQuery):
    ...
    full_project = "Склад / Общее"
    if idx_str.isdigit() and int(idx_str) < len(projects):
        full_project = projects[int(idx_str)]
    ...
    await callback.message.edit_text(
        ...
        f"🎯 Проект: {html.escape(full_project)}\n"
...
async def cmd_shift_menu(message: types.Message):
    ...
    if user_id not in active_shifts:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text=f"🟢 {p[:30]}", callback_data=f"startshift_{idx}")]
                for idx, p in enumerate(projects)
            ]
        )
        await message.answer(
...
async def cb_start_shift(callback: types.CallbackQuery):
    ...
    active_shifts[callback.from_user.id] = {
        "project": full_project,
    ...
    await callback.message.edit_text(
        "🟢 <b>Смена начата!</b>\n\n"
        f"🎯 Проект: <b>{html.escape(full_project)}</b>\n"
...
async def cb_meal_shift(callback: types.CallbackQuery):
    ...
    await log_expense(
        ...
        amount=2500,
        employee=employee,
        comment="Обеденные во время смены",
    )
...
async def cb_end_shift(callback: types.CallbackQuery):
    ...
    msg = (
        "🔴 <b>Смена завершена!</b>\n\n"
        f"🎯 Проект: {html.escape(shift['project'])}\n"
...
async def render_project_card(target, data: dict):
    ...
    text = (
        ...
        f"{transcript_block}"
        f"📅 <b>Дата мероприятия:</b> {html.escape(str(event_date))}\n"
        f"🎯 <b>Название:</b> {html.escape(str(proj_name))}\n"
        f"📍 <b>Локация / Площадка:</b> {html.escape(str(location))}\n"
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text="🎙 Наговорить заново", callback_data="admin_voice_project"),
                InlineKeyboardButton(text="✏️ Исправить поле", callback_data="edit_voice_fields_menu"),
            ],
...
async def process_voice_project(message: types.Message, state: FSMContext):
    ...
    Верни СТРОГО JSON следующей структуры:
    {
      ...
      "location": "площадка, ресторан или локация",
      "price": 1500000,
      "prepay_percent": 50
    }
    """
...
async def cb_confirm_voice_proj(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="⬅️ В панель управления", callback_data="admin_hub")]]
        )

        await callback.message.edit_text(
            f"✅ <b>Мероприятие создано!</b>\n\n"
            f"🎯 <b>{html.escape(full_proj_name)}</b>\n"
            f"📅 Дата: {html.escape(str(data.get('event_date', '')))}\n"
            f"📍 Локация: {html.escape(str(data.get('location', 'Площадка')))}\n"
            f"💵 Смета: {float(data.get('price', 0)):,.0f} ₸\n"
...
async def cb_admin_payouts_list(callback: types.CallbackQuery):
    ...
    if not debts:
        ...
        for idx, (person, sum_amt) in enumerate(debts.items()):
            ...
            buttons.append([
                InlineKeyboardButton(
                    text=f"💸 Погасить: {person[:20]} ({sum_amt:,.0f} ₸)",
                    callback_data=f"pay_{idx}",
                )
            ])
...
async def cb_pay_person_preview(callback: types.CallbackQuery):
    ...
    reqs = await get_employee_requisites(full_person_name)

    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"✅ Переведено {amount:,.0f} ₸ (Списать)",
                    callback_data=f"confirmpay_{idx_str}",
                )
            ],
...
async def cb_confirm_payment(callback: types.CallbackQuery):
    ...
    if idx_str.isdigit() and int(idx_str) < len(persons_list):
        full_person_name = persons_list[int(idx_str)]
        await batch_settle_debts(full_person_name)
...
async def handle_photo(message: types.Message):
    ...
    if photo.file_unique_id in saved_receipt_file_ids:
        await message.answer(
            "⚠️ <b>Этот чек уже был успешно внесен в таблицу ранее!</b> Повторная запись отменена.",
            parse_mode="HTML",
        )
        return
    ...
    try:
        ...
        merchant = str(data.get("merchant", "Неизвестно"))
        employee = await get_or_register_employee(message.from_user)
        ...
        if duplicate:
            ...
            await status_msg.edit_text(
                ...
                f"🎯 Проект: {html.escape(str(duplicate['project']))}\n"
...
async def process_project_choice(callback: types.CallbackQuery):
    ...
    projects = await get_active_projects()
    if choice_code == "general":
        full_project = "Склад / Общее"
    elif choice_code.isdigit() and int(choice_code) < len(projects):
        full_project = projects[int(choice_code)]
    else:
        full_project = "Склад / Общее"
    ...
    try:
        ...
        await status_update.edit_text(
            ...
            f"👤 Сотрудник: {html.escape(receipt['user_name'])}",
            parse_mode="HTML",
        )
    except Exception as e:
